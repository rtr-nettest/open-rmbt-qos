 ...
 ******************************************************************************/
package at.rtr.rmbt.qos.testserver.service;
...
public class ServiceManager {
	...
	public void addService(AbstractJob<?> service) {
		if (service instanceof IntervalJob<?>) {
			...
			service.setCallback(AbstractJob.JobState.ERROR, new JobCallback () {
				
				@Override
				public boolean onEvent(AbstractJob<?> service, AbstractJob.JobState state, Object newResult) {
					if (AbstractJob.JobState.ERROR.equals(state) && ((IntervalJob<?>) service).restartOnError()) {
						synchronized (serviceMap) {
							...
							if (fs != null && fs.getService().getIsRunning()) {
								...
								if (((IntervalJob<?>) service).getExecutionCounter() >= 1) {
									newService.setCallback(AbstractJob.JobState.ERROR, this);
								}
								else {
	 ...
	 * @param type
	 */
	public void dispatchEvent(EventJob.EventType type) {
