							...
							if (!isCommandLine) {
								...
								case DEBUG_COMMAND_LIST_SERVICES:
									synchronized (TestServer.getInstance().serviceManager.getServiceMap()) {
										Iterator<Entry<String, ServiceManager.FutureService>> entries = TestServer.getInstance().serviceManager.getServiceMap().entrySet().iterator();
										printlnCommand("\n" + TestServer.getInstance().serviceManager.getServiceMap().size() + " services found:\n");
										int i = 0;
										while (entries.hasNext()) {
											Entry<String, ServiceManager.FutureService> e = entries.next();
